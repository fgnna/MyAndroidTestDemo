...
int eagle_test(int a, char* string);
HOOK_INFO custom_hook_info_test = {{}, "libcocos2dlua", "_Z13xxtea_decryptPhjS_jPj", eagle_test, eagle_test};
...
int eagle_test(int a, char* string){
	...
	hook_postcall(&eph);
	int uid = getuid();
	LOGI("{\"Basicxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\":[\"%d\",\"%d\",\"false\"],\"InvokeApi\":{\"%s\":{\"a\":\"%d\",\"string\":\"%s\"},\"return\":{\"int\":\"%d\"}}}",
		uid, NATIVE_APP_API, "test", a, string, res);
	return res;
}
